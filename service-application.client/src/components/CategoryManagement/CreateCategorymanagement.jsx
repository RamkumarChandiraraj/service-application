...
function CreateCategoryManagement() {
  ...
  useEffect(() => {
    ...
    const fetchCategory = async () => {
      setPageLoading(true);
      try {
        const data = await getCategoryById(id); // data is already the inner object
        setFormData({
          name: data.name || "",
  ...
  const validate = () => {
    ...
    if (!formData.icon.trim()) temp.icon = "Icon is required";
      ...
      <div className="w-50 rounded bg-white border shadow p-4">
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="d-flex justify-content-end">
            <Link to="/management/categories" className="btn btn-secondary me-2">
              Cancel
            </Link>
